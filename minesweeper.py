...
class GameGrid:
    ...
    def neighbors(self, i, j):
        ...
        for x in range(j-1,j+2):
            for y in range(i-1, i+2):
                print(x,y)
                if 0<=x<=self.ncols-1 and 0<=y<=self.nrows-1 and (x,y) != (j,i):
                    l.append((y,x))
        return l
            
        
        
        # if (i + 1 <= self.nrows-1) and (i - 1 >= 0) and (j + 1 <= self.ncols-1) and (j-1 >= 0):
        #     return [(i, j+1), (i, j-1), (i+1, j), (i-1, j), (i+1, j+1), (i-1, j-1), (i+1, j-1), (i-1, j+1)]
        # elif (i > self.nrows-1) or (i < 0) or (j > self.ncols-1) or (j < 0):
        #     return None
        # elif (i == self.nrows-1) and (i - 1 >= 0) and (j + 1 <= self.ncols-1) and (j-1 >= 0):
        #     return [(i, j+1), (i, j-1), (i-1, j), (i-1, j-1), (i-1, j+1)]
        # elif (i == self.nrows-1) and (i - 1 >= 0) and (j == self.ncols-1) and (j-1 >= 0):
        #     return [(i, j-1), (i-1, j), (i-1, j-1)]
        # elif (i + 1 == self.nrows-1) and (i - 1 >= 0) and (j + 1 <= self.ncols-1) and (j-1 == 0):
        #     return [(i, j+1), (i-1, j), (i-1, j+1)]
        # elif (i + 1 <= self.nrows-1) and (i == 0) and (j + 1 <= self.ncols-1) and (j-1 >= 0):
        #     return [(i, j+1), (i, j-1), (i+1, j), (i+1, j+1), (i+1, j-1)]
        # elif (i + 1 <= self.nrows-1) and (i == 0) and (j == self.ncols-1) and (j-1 >= 0):
        #     return [(i, j-1), (i+1, j), (i+1, j-1)]
        # elif (i + 1 <= self.nrows-1) and (i == 0) and (j + 1 <= self.ncols-1) and (j== 0):
        #     return [(i, j+1), (i+1, j), (i+1, j+1)]
        # elif (i + 1 <= self.nrows-1) and (i - 1 >= 0) and (j == self.ncols-1) and (j-1 >= 0):
    ...
    def flags_nearby(self, i, j):
        ...
        for (a, b) in self.neighbors(i,j):
            if self.square_at(a,b).flagged is True:
                n += 1
        return n
    #--------------------------------------------------------------------------
    ...
    def place_mine(self, i, j):
        ...
        """
        if self.square_at(i, j).mined is True:
            return
        else:
            self.square_at(i,j).mined = True
            for (a, b) in self.neighbors(i, j):
                s = self.square_at(a,b)
                s.mines_nearby += 1
            self.nmines += 1
    ...
    def reveal(self, i, j):
        ...
        """
        if self.revealed(i,j) is True or self.flagged(i,j) is True:
            return
        else:
            self.square_at(i,j).revealed = True
            self.squares_revealed +=1
            if self.mined(i,j) != True and self.mines_nearby(i,j) == 0:
                for (a,b) in self.neighbors(i, j):
                    self.reveal(a,b)
    ...
    def random_game(self, n_mines):
        ...
        for r in range(0,self.nrows):
            for c in range(0,self.ncols):
                ...
                l.append(point)
        print(l)
        rmines = random.sample(l, n_mines)
        print(rmines)
        for (a,b) in rmines:
            self.place_mine(a,b)
...
class MinesweeperApp:
    ...
    def left_click_handler(self, i, j):
        ...
        if self.game.square_at(i,j).flagged == True:
            return
        
        if self.game.square_at(i,j).revealed == True:
            self.game.chording(i,j)
        else:
            self.game.reveal(i,j)
            
        if self.game.game_lost() == True:
    ...
    def right_click_handler(self, i, j):
        ...
        if self.game.flagged(i,j) == False:
            self.game.place_flag(i,j)
        else:
            self.game.square_at(i,j).flagged = False
            self.game.nflags -= 1
        self.draw_cell(i,j)
